...
TITLE_PREFIX_LEVEL = re.compile(
    r"(?i)^\s*(?:(?:资深|高级|中级|初级|senior|junior|sr\.?|jr\.?)\s*)+"
)
...
class EvolutionEngine:
    ...
    def run(self) -> dict[str, Any]:
        ...
        try:
            ...
            if self.data_source is not None:
                ...
                descriptions = self.data_source.load_descriptions(
                    jd_id
            ...
            role_candidates = self._discover_roles(
                jds,
                known_titles,
                role_company,
                skill_company,
    ...
    def _discover_roles(
        ...
        for members in concept_members.values():
            ...
            raw_title = (
                stat["current_raw_titles"].most_common(1)[0][0]
                if stat["current_raw_titles"]
                else stat["raw_titles"].most_common(1)[0][0]
            )
