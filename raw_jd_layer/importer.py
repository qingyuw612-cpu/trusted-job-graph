...
def prepare_row(record: dict[str, Any], context: FileContext, now: str) -> dict[str, Any]:
    ...
    content_hash = stable_hash(
        json.dumps(version_properties, ensure_ascii=False, sort_keys=True, separators=(",", ":")),
        64,
    )
...
BATCH_QUERY = """
MATCH (source:RawSourceFile {source_file_id:$source_file_id})
MATCH (run:RawIngestionRun {run_id:$run_id})
UNWIND $rows AS row
MERGE (job:RawJob {raw_uid:row.raw_uid})
ON CREATE SET job.first_seen_at = $now
SET job.last_seen_at = $now
MERGE (version:RawJDVersion {version_id:row.version_id})
ON CREATE SET version.created_at = $now, version.first_ingested_at = $now
SET version += row.version_props,
    version.last_seen_at = $now,
    version.last_ingest_run_id = $run_id
MERGE (job)-[:HAS_VERSION]->(version)
MERGE (version)-[:FROM_SOURCE]->(source)
MERGE (job)-[:SEEN_IN_SOURCE]->(source)
...
FOREACH (_ IN CASE WHEN make_current THEN [1] ELSE [] END |
    SET job += row.job_props,
        job.current_version_id = row.version_id,
