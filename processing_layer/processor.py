...
WHERE coalesce(raw.processing_version, '') <> $processor_version
...
FOREACH (review IN row.reviews |
    MERGE (task:ProcessingReview {review_id: review.review_id})
    SET task += review.properties
    MERGE (processed)-[:HAS_REVIEW]->(task)
)
...
def processing_row(
    ...
    llm_extractor: WebhookLLMExtractor | None = None,
) -> dict[str, Any]:
